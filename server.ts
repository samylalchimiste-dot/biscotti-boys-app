...
function sanitizeSettings(settings: any): any {
  ...
  if (copy.instagramUrl) {
    const ig = String(copy.instagramUrl).trim();
    if (ig.includes('ais-dev-') || ig.includes('ais-pre-') || /north47|shelf|tricoma|pornhub|xvideos|sex|porn|onlyfans|adult/i.test(ig)) {
      copy.instagramUrl = '';
    }
  }
  if (copy.instagramUrl2) {
    const ig2 = String(copy.instagramUrl2).trim();
    if (ig2.includes('ais-dev-') || ig2.includes('ais-pre-') || /north47|shelf|tricoma|pornhub|xvideos|sex|porn|onlyfans|adult/i.test(ig2)) {
...
async function setupTelegramWebhook() {
  ...
  try {
    if (!isMenuButtonMethodFrozen) {
      ...
      // Ensure Telegram Bot Name, Bio and Description are completely sanitized and locked across all languages
      await ensureTelegramBotProfileIsClean(token);
    }
  } catch (err) {
...
async function ensureTelegramBotProfileIsClean(token: string) {
  if (!token) return;
  const officialName = 'BISCOTTI BOYS FARM';
  const officialDesc = "💎 BISCOTTI BOYS FARM — Réserve Privée Officielle 💎\nExtractions d'exception, sélections d'élite & catalogue VIP exclusif.";
  const officialShortDesc = "BISCOTTI BOYS FARM — Réserve Privée Officielle. Sélections & Extractions d'exception.";
  const langCodes = ['', 'fr', 'en', 'es', 'de', 'ar', 'ru', 'it', 'pt'];
  ...
  try {
    ...
    // 2. Lock Descriptions & Bio across all languages
    for (const lang of langCodes) {
      fetch(`https://api.telegram.org/bot${token}/setMyDescription`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ description: officialDesc, language_code: lang })
      }).catch(() => {});

      fetch(`https://api.telegram.org/bot${token}/setMyShortDescription`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ short_description: officialShortDesc, language_code: lang })
      }).catch(() => {});
    }
    console.log('[TELEGRAM] Bot bio, name, and descriptions secured and locked.');
  } catch (e) {
    console.warn('[TELEGRAM] Failed to set bot profile descriptions:', e);
  }
}

// Periodic guardian to prevent any unauthorized description or bio tampering
setInterval(() => {
  const token = getTelegramBotToken();
  if (token) {
    ensureTelegramBotProfileIsClean(token).catch(() => {});
  }
}, 30 * 60 * 1000);
